class HeadlinesController < ApplicationController
  ...
  # GET /headlines.json
  def index
    @headlines = if params[:query].present?
      Headline.search(params[:query])
    else
      Headline.all
    end
  end
  ...
  # def autocomplete
  #   render json: Headline.search(params[:query], autocomplete: true, limit: 10).map(&:title)
  # end

  def autocomplete
    json = Headline.search(params[:query], autocomplete: true, limit: 10).map do |headline|
      { title: headline.title, value: headline.id }
    end
    logger.info "*" * 80
    logger.info json
    logger.info "*" * 80
